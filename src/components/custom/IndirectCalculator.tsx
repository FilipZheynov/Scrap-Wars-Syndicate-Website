...
import { generateIndirectResults } from "./calculatorEngine";
...
export const IndirectCalculator = () => {
  const [tntKg, setTntKg] = useState(10);
  const rows = useMemo(() => generateIndirectResults(tntKg), [tntKg]);
    ...
    <section className="rounded-2xl border border-zinc-800 bg-zinc-950/70 p-4 md:p-6">
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
        <label className="flex flex-col gap-2">
          <span className="text-xs font-bold uppercase tracking-widest text-zinc-400">TNT equivalent (kg)</span>
          <input className={inputClassName} type="number" min={1} max={100} value={tntKg} onChange={(e) => setTntKg(Number(e.target.value) || 0)} />
        </label>
      </div>
